...
description: |
  小红书自动化技能集合。支持内容创作、图片渲染、认证登录、内容发布、搜索发现、社交互动、复合运营。
...
1. **内容创作**（"写笔记 / 创作小红书内容 / 生成图片卡片"）→ 执行`xhs-content-creation`技能。
2. **认证相关**（"登录 / 检查登录 / 切换账号"）→ 执行 `xhs-auth` 技能。
3. **内容发布**（"发布 / 发帖 / 上传图文 / 上传视频"）→ 执行 `xhs-publish` 技能。
...
### xhs-content-creation — 内容创作

撰写小红书笔记内容、生成 Markdown 文档、渲染图片卡片（支持 8 套主题、4 种分页模式）。
